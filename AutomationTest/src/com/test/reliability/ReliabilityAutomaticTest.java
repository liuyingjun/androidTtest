...
public class ReliabilityAutomaticTest extends AcceptanceTestCase {
    private static final String TAG = "ReliabilityTest";

    private static ReliabilityAutomaticModules tc = null;
    ...
    protected void setUp() throws Exception {
        ...
        if (!hasLoadConfigFile) {
            TestDataExtract.loadConfigFile(this, "ReliabilityConfig.xml");
            hasLoadConfigFile = true;
        }
        if (tc == null) {
            tc = new ReliabilityAutomaticModules(this);
        }
    }
    ...
    public void test_106() throws IllegalArgumentException, ClassNotFoundException,
        ...
        Log.d(TAG, "^^^^^^^^^^^^^" + TestDataExtract.getCasename(106) + "|" + sCurrLoop
                + "^^^^^^^^^^^^^^");
        printSnarioNameInResult(sCurrParameter.get("snarioname"));
    ...
    @SuppressWarnings("static-access")
    public void test_111() throws IllegalArgumentException, ClassNotFoundException,
            NoSuchMethodException, IllegalAccessException, InvocationTargetException {
        Log.d(TAG, "===============Begin test_092===============");

        this.sCurrParameter = TestDataExtract.getParameters(111);
        sCurrLoop = ++loopList[111];
        Log.d(TAG, "^^^^^^^^^^^^^" + TestDataExtract.getCasename(111) + "|" + sCurrLoop
                + "^^^^^^^^^^^^^^");
        printSnarioNameInResult(sCurrParameter.get("snarioname"));
        printCaseNameInResult(TestDataExtract.getCasename(111));
        fieldMethods(TestDataExtract.getCasename(111), tc, sCurrParameter);
        Log.d(TAG, "================End test_092================");
    }
    ...
    public void test_113() throws IllegalArgumentException, ClassNotFoundException,
        ...
        this.sCurrParameter = TestDataExtract.getParameters(113);
        sCurrLoop = ++loopList[113];
        Log.d(TAG, "^^^^^^^^^^^^^" + TestDataExtract.getCasename(113) + "|" + sCurrLoop
                + "^^^^^^^^^^^^^^");
        printSnarioNameInResult(sCurrParameter.get("snarioname"));
        printCaseNameInResult(TestDataExtract.getCasename(113));
        fieldMethods(TestDataExtract.getCasename(113), tc, sCurrParameter);
        Log.d(TAG, "================End test_094================");
    }
    ...
    private void fieldMethods(String methodName, ReliabilityAutomaticModules nts, HashMap<String, String> paras)
            throws ClassNotFoundException, NoSuchMethodException, IllegalArgumentException,
            IllegalAccessException, InvocationTargetException {
        Class clazz = Class.forName("com.test.reliability.ReliabilityAutomaticModules");
