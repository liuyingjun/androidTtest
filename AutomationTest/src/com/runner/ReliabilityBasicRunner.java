...
import com.test.reliability.ReliabilityAutomaticTest;
...
public class ReliabilityBasicRunner extends UiAutomatorInstrumentationTestRunner {
    ...
    public TestSuite getAllTests() {
        ...
        boolean isSuccess = TestDataExtract.loadConfigFile(this, "ReliabilityConfig.xml");
        if (!isSuccess) {
            return suite;
        }
        ReliabilityAutomaticTest.hasLoadConfigFile = true;
        ...
        try {
            for (int j = 0; j < Integer.parseInt(TestDataExtract.totalLoops); j++) {
                for (int k = 0; k < casesSize; k++) {
                    ...
                    ReliabilityAutomaticTest.sRunLoops = times;
                    Log.d(TAG, "The current case run " + times + " times");
                    String method = methodNameList.get(k);

                    for (int n = 0; n < times; n++) {
                        suite.addTest((new TestMethod(method, ReliabilityAutomaticTest.class)).createTest());
                    }
                }
            }
