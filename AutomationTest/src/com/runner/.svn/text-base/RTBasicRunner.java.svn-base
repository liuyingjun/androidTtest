...
public class RTBasicRunner extends UiAutomatorInstrumentationTestRunner{
    ...
    public TestSuite getAllTests() {
        ...
        boolean isSuccess = TestDataExtract.loadConfigFile(this, "RTConfig.xml");
        if (!isSuccess) {
            return suite;
        }
            ...
            for (int j = 0; j < Integer.parseInt(TestDataExtract.totalLoops); j++) {
                for (int k = 0; k < casesSize; k++) {
                    ...
                    for (int n = 0; n < times; n++) {
                        suite.addTest((new TestMethod(method, RTAutomaticTest.class)).createTest());
                    }
                }
            }
